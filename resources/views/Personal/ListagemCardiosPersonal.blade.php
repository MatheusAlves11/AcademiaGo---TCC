    ...
    <body>
        <div class="container bg-light " style="width: 90%;text-align: center;">
            ...
            <div class="row ">
                ...
                <div class="col-sm-10">
                    <h2 class="texto mb-4"><b>Cardios Cadastrados</b></h2>
                    <form class="forms-pesquisa" action="/homePersonal-cardio" method="get">
                        <input type="text" name="search" class="input form-control me-2" placeholder="Procure cárdio" id="search">
                    </form>

                    <div class="table-responsive rolagem">
                        @if (empty($cardios) && $busca)
                        <p><a href="/homePersonal-cardio">Cardio não encontrado</a></p>
                        <div class="table-responsive rolagem">
                            @else
                            <table class="table">
                                <thead>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{$cardio->exercicio}}</td>
                                        <td>{{$cardio->duracao}} {{$cardio->tipoTempoDuracao}}</td>
                                        <td title = "*estimativa">{{$cardio->meta}}</td>
                                        <td style="display: flex;flex-direction: row;justify-content: space-around;align-items: center;">
                                            <a href="editarCardio/{{$cardio->id}}" class="btn btn-primary" style="height: auto">Editar</a>
                                            <form action="/destruirCardio/{{$cardio->id}}" method="POST">
                                                @csrf
                                                @method('DELETE')
                        ...
                        <a href="/cadastroCardio" class="btn btn-primary">Cadastrar</a>
                    </div>
                </div>
            </div>
